!=============================================================================
! parma_subroutines.f90 — standalone PARMA v4.10 subroutines (Sato, JAEA)
!
! Source: EXPACS standalone distribution, PARMA v4.10 (2021-03-21)
!         http://phits.jaea.go.jp/expacs
!
! Copyright (c) 2006 Japan Atomic Energy Agency (JAEA). All Rights Reserved.
!
! Conditions for use (EXPACS terms, https://phits.jaea.go.jp/expacs/):
!   1. Free to use, redistribute, and modify for NON-COMMERCIAL purposes.
!   2. Commercial use requires prior agreement with JAEA.
!   3. Any publication using results from this code MUST cite:
...
!      and acknowledge the URL: http://phits.jaea.go.jp/expacs
!   4. Send a copy of any publication to: nsed-expacs@jaea.go.jp
!   5. JAEA is not liable for any loss or damage from use of this software.
!
! Modifications for UCMuon (no physics changes):
!   - Added 'use parma_path' to subroutines that open data files.
!   - file='input/...' replaced by trim(parma_datadir)//'input/...'
!     so the data directory is configurable via parma_set_datadir().
!=============================================================================
! *******************************************************
...
if(ifirst.eq.0) then
 ...
 do i=0,5
  if(i.le.2) then ! neutron, proton, alpha
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/Rigid-Dep.inp',status='old')
  elseif(i.eq.3) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/Rigid-Dep-EL.inp',status='old')
  elseif(i.eq.4) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/Rigid-Dep-PO.inp',status='old')
  elseif(i.eq.5) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/Rigid-Dep-PH.inp',status='old')
  endif
  read(15,'(a)') chatmp1
  do ib=1,nBdata
  ...
  close(unit=15)
 enddo
 open(unit=15,file=trim(parma_datadir)//'input/ions/Rigid-Dep.inp',status='old')
 do i=npart-5,npart
  read(15,'(a)') chatmp1
  do ib=1,nBdata
   read(15,1010) chatmp5,(A(i,ib,ia),ia=1,nAdata)
...
if(ifirst.eq.0) then
 ...
 do i=0,7
  if(i.le.2) then ! neutron, proton, alpha
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/solar-dep.inp',status='old')
  elseif(i.eq.3) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/solar-dep-EL.inp',status='old')
  elseif(i.eq.4) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/solar-dep-PO.inp',status='old')
  elseif(i.eq.5) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/solar-dep-PH.inp',status='old')
  elseif(i.eq.6) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/solar-dep.plus',status='old')
  elseif(i.eq.7) then
   open(unit=15,file=trim(parma_datadir)//'input/'//pname(i)//'/solar-dep.mins',status='old')
  endif
  read(15,'(a)') chatmp1
  do ib=1,nBdata
  ...
  close(unit=15)
 enddo
 open(unit=15,file=trim(parma_datadir)//'input/ions/solar-dep.inp',status='old')
 read(15,'(a)') chatmp1 
 do i=npart-5,npart
  do ib=1,nBdata
   read(15,*) (A(i,ib,ia),ia=1,nAdata)
...
if(ifirst.eq.0) then
 ...
 do i=0,npart
  if(i.eq.0) then ! neutron
   open(unit=15,file=trim(parma_datadir)//'input/neutro/bestR.inp',status='old')
  else ! electron, positron, photon
   open(unit=15,file=trim(parma_datadir)//'input/elemag/bestR-'//pname(i)//'.inp',status='old')
  endif
  read(15,'(A)') chatmp1
  do id=1,ndep
...
!      Read A parameter
 open(unit=3,file=trim(parma_datadir)//'input/neutro/fitting-lowspec.inp',status='old')
 read(3,'(a)') chatmp1
...
if(P(1).eq.0.0) then
 open(unit=3,file=trim(parma_datadir)//'input/neutro/Geo-Dep.inp',status='old')
 read(3,'(a)') chatmp1
 read(3,*) p(1),p(2),p(3)
 close(Unit=3)
 open(unit=3,file=trim(parma_datadir)//'input/neutro/Water-Dep.inp',status='old')
 read(3,'(a)') chatmp1
 read(3,1001) chatmp4,p(4),p(5),p(6)
 read(3,1001) chatmp4,p(7),p(8),p(9)
 read(3,1001) chatmp4,p(10),p(11),p(12),p(13),p(14)
 close(unit=3)
 open(unit=3,file=trim(parma_datadir)//'input/neutro/Aircraft-Dep.inp',status='old')
 read(3,'(a)') chatmp1
...
if(B(2).eq.0.0) then ! first time
 open(unit=15,file=trim(parma_datadir)//'input/neutro/Depth-Dep-mid.out',status='old')  ! read B(2)-B(4) (independent of s,r,d)
 read(15,'(a)') chatmp1
 read(15,'(a)') chatmp1
 read(15,*) tmp1,tmp2,B(2),B(3),B(4)
 close(unit=15)
 open(unit=15,file=trim(parma_datadir)//'input/neutro/Rigid-Dep.inp',status='old')
...
if(B(4).eq.0.0) then ! first time
 open(unit=15,file=trim(parma_datadir)//'input/neutro/Depth-Dep-hig.out',status='old')  ! read B(2),B(4) (independent of s,r,d)
 read(15,'(a)') chatmp1
 read(15,'(a)') chatmp1
...
! open(15,file='correction-depth-rigid-final.inp',status='old') ! only high altidue correction mode
 open(15,file=trim(parma_datadir)//'input/neutro/correction-depth-rigid.inp',status='old') ! all altitude correction mode
 read(15,'(a)') chatmp1
 do ih=1,nhensu
  do id=1,ndep
...
if(ifirst.eq.0) then  ! first time called this routine
 ifirst=1
 do ip2=1,npart
  open(15,file=trim(parma_datadir)//'input/muon--/final135.'//charge(ip2),status='old') ! read A(1),A(3),A(5), they are solar independent
  read(15,*) chatmp1
  do id2=1,ndep
   read(15,*) dep(id2),Bdata(1,1,ip2,id2),Bdata(3,1,ip2,id2),Bdata(5,1,ip2,id2)
  enddo
  close(15)
  open(15,file=trim(parma_datadir)//'input/muon--/final2467.'//charge(ip2),status='old') ! read A(2),A(4),A(6),A(7)
  read(15,*) chatmp1
  do id2=1,ndep
...
if(ifirst.eq.0) then  ! first time call
 ifirst=1
 open(unit=15,file=trim(parma_datadir)//'input/ions/Combine.inp',status='old')
 read(15,*) chatmp1
 do i=1,ngroup
...
if(ifirst.eq.0) then
 do i=1,5 ! proton, alpha, electron, positron, photon
  if(i.eq.1) then
   open(unit=15,file=trim(parma_datadir)//'input/proton/fitting-lowspec.inp',status='old')
  elseif(i.eq.2) then
   open(unit=15,file=trim(parma_datadir)//'input/alphaa/fitting-lowspec.inp',status='old')
  elseif(i.eq.3) then
   open(unit=15,file=trim(parma_datadir)//'input/elemag/fitting-lowspec-EL.inp',status='old')
  elseif(i.eq.4) then
   open(unit=15,file=trim(parma_datadir)//'input/elemag/fitting-lowspec-PO.inp',status='old')
  elseif(i.eq.5) then
   open(unit=15,file=trim(parma_datadir)//'input/elemag/fitting-lowspec-PH.inp',status='old')
  endif  
  read(15,'(A)') chatmp1
  do id=1,ndep
  ...
  close(unit=15)
 enddo
 open(15,file=trim(parma_datadir)//'input/ions/fitting-lowspec.inp',status='old')
 read(15,'(a)') chatmp1
 do i=npart-5,npart
...
if(down.eq.0.0) then ! first time call this routine
 down=1.720313d0
 do ig=1,ngroup
  open(unit=15,file=trim(parma_datadir)//'input/ions/primary-'//gname(ig)//'.inp',status='old')
  read(15,*) chatmp1
  do id=1,ndep
  ...
  close(15)
 enddo
 open(15,file=trim(parma_datadir)//'input/ions/dEdx-table.inp',status='old')
 read(15,'(a1)') chatmp1
 read(15,'(a1)') chatmp1
...
if(ifirst.eq.0) then
 ifirst=1
 open(15,file=trim(parma_datadir)//'input/elemag/flux511keV.inp')
 read(15,'(a1)') chatmp1
 do id=1,ndep
...
if(ifirst.eq.0) then ! come to this routine first, so read input data
 ifirst=1
 open(unit=15,file=trim(parma_datadir)//'input/AtomDepth.inp',status='old')
 read(15,'(a)') chatmp1
...
if(ifirst.eq.0) then ! come to this routine first, so read input data
 ifirst=1
 open(unit=15,file=trim(parma_datadir)//'input/CORdata.inp',status='old')
...
if(ip.eq.4.and.((g.ge.0.0d0.and.g.le.1.0d0).or.g.eq.100.0d0).and.ang.lt.coshigh) then ! ground level muon, condition for angle above 45 deg is introduced in 2024/11/10
...
if(ifirst.eq.0) then ! first time call this routine
 ifirst=1
 do ip1=1,npart
  open(15,file=trim(parma_datadir)//'input/angle/'//pname(ip1)//'.out',status='old')
  read(15,'(a1)') chatmp1
  do i=1,maxfit
   do is=1,nsur
    do ic=1,ncor
  ...
  enddo
  close(15)
  open(15,file=trim(parma_datadir)//'input/angle/'//pname(ip1)//'-Eint.out',status='old')
  read(15,'(a1)') chatmp1
  do is=1,nsur
   do ic=1,ncor
...
! Read Ground Correction Factor
 open(15,file=trim(parma_datadir)//'input/angle/NeutronGround.out',status='old')
 do i=1,maxfit
...
! Read Black hole factor parameter (only for neutron, elepos, photon)
 do ip2=1,npart
  open(15,file=trim(parma_datadir)//'input/angle/BkH-'//pname(ip2)//'.inp',status='old')
  read(15,'(a1)') chatmp1
  do i=1,nBHpara
...
if(ifirst.eq.0) then ! first time call this subroutine
 do iy=iymin,iymax  ! Initialized
  ...
  enddo
 enddo
 open(unit=15,file=trim(parma_datadir)//'input/FFPtable.day',status='old')
 ...
 enddo
 close(unit=15)
 open(unit=15,file=trim(parma_datadir)//'input/FFPtable.uso',status='old')
 read(15,*) iysUs,iyeUs
 do iy=iysUs,iyeUs
...
if(FFP(iy0,im0,id0).gt.-99.0) then ! data exist
 ...
 if(iy0.ge.1951) return ! after modern space observation started
endif

!  ***** Determine FFP from Usoskin's data  *****
if(FFPuso(iy0).gt.-99.0) then
 getHP=FFPuso(iy0)

...
│   ├── codes/                    per-code setup + README (geant4, phits, music, …)
│   ├── analysis/                 cross-code scripts (benchmark_analysis.py, …)
│   ├── results/                  summary CSVs + timing files
│   ├── figures/v2_six_code/      canonical comparison plots
│   └── reports/                  BENCHMARK_FEEDBACK.md (the v2 reference)
├── manuscript/scripts/           one script per figure of the paper
│
├── examples/
...
UCMuon has been validated against Geant4 11.2, PHITS 3.36, and MUSIC across monoenergetic and realistic-spectrum test suites. The benchmark suite is in `benchmark/`: the per-code setups (`codes/`), the cross-code analysis scripts (`analysis/`), the summary CSVs and timing files (`results/`), the canonical figures (`figures/v2_six_code/`), and the reference write-up (`reports/BENCHMARK_FEEDBACK.md`). `benchmark/README.md` gives the roadmap for reproducing it end to end. The ~8 GB of raw per-event output is not tracked; it is regenerated by the per-code steps.

`manuscript/scripts/` holds one script per figure of the paper, so every figure can be regenerated from the code and these benchmark products.

A quick check that the installation itself works is `bash test_run/run_test.sh` (see [Verify the installation](#verify-the-installation)).

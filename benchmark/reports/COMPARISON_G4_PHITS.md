...
For the full six-code benchmark, which is the one the paper reports, see
**[BENCHMARK_FEEDBACK.md](BENCHMARK_FEEDBACK.md)**.
...
```bash
$PHITS_HOME/bin/phits.sh muon_rock.inp     # wherever PHITS is installed
```
...
```bash
cd benchmark/geant4_muon_rock_v5     # the git-ignored scratch tree
...
└── BENCHMARK_FEEDBACK.md                      ← the six-code reference run
```

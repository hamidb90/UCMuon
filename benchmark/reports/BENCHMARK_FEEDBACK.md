...
- ⚠️ **UCMuon-MC: 54–61% of Geant4** — similar to BB.
  MCS angles unchanged from v1 (only position tracking was fixed).
  **[Resolved 2026-07-05, eef885c]** Not a PUMAS issue: UCMuon-MC is a native
  stochastic engine, not PUMAS-based. The "similar to BB" note above was the
  clue: both engines shared a √2 MCS under-scatter, because Highland's
  θ₀ is the RMS of the *projected* angle but the polar deflection was drawn
  from N(0, θ₀). Both now sample Rayleigh(θ₀).
...
| 🟡 Medium | UCMuon-MC energy loss at 2.65 MWE is −12% vs Geant4 (0.572 vs 0.649 GeV). Investigate threshold/cutoff effect at shallow depths in the UCMuon-MC loss model (not PUMAS; see the angle note above). | UCMuon-MC | Open |
